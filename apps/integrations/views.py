...
from .actions import get_integration, get_org_id_name_from_access_token
...
class IntegrationsView(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        try:
            access_token = self.request.META.get('HTTP_AUTHORIZATION').split(' ')[1]
            org_id, org_name = get_org_id_name_from_access_token(access_token)

            serializer.save(org_id=org_id, org_name=org_name)
        except Exception as error:
            logger.info(error)
            raise AuthenticationFailed('Invalid access token')
